...
Initialisie a VM object:
 ...
 my $hypervisor   = VM::JiffyBox->new($secret_token);
...
Do stuff with a specific box:
 ...
 my $ip    = $vm->get_information( filter => 'ip_address' );

 my $image = $vm->get_backup_id( max_old => '24h' );

Forther stuff which is needed:

 # tell a box a specific API-Token,
 # represented with the hypervisor
 $vm->set_hypervisor( $jb );
...
This is an example how we then could use this interface:
 ...
 my $jiffy        = VM::JiffyBox->new( 'geheim' );

 my $box_id_A     = $jiffy->get_id_for_name( 'Kunde_A' );

 my $box_kA       = $jiffy->get_vm( $box_id_A );

 my $backup_img_A = $box_kA->get_backup_id();

 my $test_box_A   = $jiffy->create_vm( $backup_img_A );

 my $test_ip_A    = $test_box_A->get_information( filter=>'ip_address' ); 
